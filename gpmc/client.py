...
class Client:
    ...
    def _search_for_media_files(self, path: str | Path, recursive: bool) -> list[Path]:
        ...
        if path.is_file():
            # Check MIME type but with manual fallback for common video extensions
            mimetype_guess = mimetypes.guess_type(path)[0]
            
            # If MIME type is detected and valid, use it
            if mimetype_guess and any(mimetype_guess.startswith(mimetype) for mimetype in self.valid_mimetypes):
                return [path]
            ...
            # If neither MIME type nor extension suggests it's a video/image, reject
        ...
        # Filter media files with MIME type + extension fallback
        media_files = []
        video_extensions = {'.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm', '.m4v', '.mpg', '.mpeg', '.ts', '.mts', '.m2ts'}
        image_extensions = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.webp'}
        
        for file in files:
            mimetype_guess = mimetypes.guess_type(file)[0]
            
            # Check MIME type first
            if mimetype_guess and any(mimetype_guess.startswith(mimetype) for mimetype in self.valid_mimetypes):
                media_files.append(file)
            # Fallback to extension check
            elif file.suffix.lower() in video_extensions or file.suffix.lower() in image_extensions:
                media_files.append(file)
